<?php
	...
	*/
	...
	class Cashier1{
		...
		protected static $tbl_name = "counters";
		function db_fields(){
			global $mydb;
		...
		function list_of_cashier1KioskTransactions(){
			global $mydb;
			$mydb->setQuery("SELECT t.queue_number, c.counter_name
							FROM ".self::$tbl_name." c
							JOIN transactions t on c.counter_id = t.counter_id 
							WHERE DATE(date_created) = CURDATE() AND t.status = 'Pending' AND c.counter_name = 'cashier1'
		...
		function get_currentQueueNumber(){
		    global $mydb;
		...
		function get_currentLastCompletedQueueNumber(){
		    global $mydb;
